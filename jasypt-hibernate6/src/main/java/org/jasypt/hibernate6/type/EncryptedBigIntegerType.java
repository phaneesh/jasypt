...
import org.jasypt.hibernate6.encryptor.HibernatePBEBigIntegerEncryptor;
import org.jasypt.hibernate6.encryptor.HibernatePBEEncryptorRegistry;

/**
 * A <b>Hibernate</b> <tt>UserType</tt> implementation which allows transparent encryption of
 * BigInteger values during persistence of entities.
 *
 ...
 * </pre>
 *
 * <p>...where a <tt>HibernatePBEBigIntegerEncryptor</tt> object should have been previously
 * registered to be used from Hibernate with name <tt>myHibernateBigIntegerEncryptor</tt> (see
...
public final class EncryptedBigIntegerType implements UserType, ParameterizedType {
  ...
  public Object nullSafeGet(
      ...
      throws SQLException {
      checkInitialization();
      final BigDecimal decimalMessage = rs.getBigDecimal(position);
      if (rs.wasNull()) {
          return null;
      }
      final BigInteger message =
          decimalMessage.setScale(0, RoundingMode.UNNECESSARY).unscaledValue();
  ...
  public void nullSafeSet(
    ...
    if (value == null) {
      ...
      final BigInteger encryptedMessage = this.encryptor.encrypt((BigInteger) value);
    ...
    if (paramEncryptorName != null) {
      ...
      if ((paramAlgorithm != null)
        ...
        throw new EncryptionInitializationException(
            "If \""
                + ParameterNaming.ENCRYPTOR_NAME
                + "\" is specified, none of \""
                + ParameterNaming.ALGORITHM
                + "\", \""
                + ParameterNaming.PASSWORD
                + "\" or \""
  ...
  private synchronized void checkInitialization() {
    if (!this.initialized) {
      if (this.useEncryptorName) {
        ...
        final StandardPBEBigIntegerEncryptor newEncryptor = new StandardPBEBigIntegerEncryptor();
